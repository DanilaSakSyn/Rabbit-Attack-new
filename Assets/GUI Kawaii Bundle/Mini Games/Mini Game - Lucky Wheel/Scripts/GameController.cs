...
namespace FortuneWheel
{
    public class GameController : MonoBehaviour
    {
        ...
        public void TurnWheel()
        {
            ...
            int currentGold = Wallet.Instance != null ? Wallet.Instance.Coins : totalGold;

            if (currentGold >= turnCost) // If player has enough gold to turn the wheel
            {
                ClaimTurnCost();
        ...
        private void ClaimTurnCost()
        {
            if (Wallet.Instance != null)
            {
                ...
                totalGold = Wallet.Instance.Coins; // Синхронизируем локальную переменную
            }
            else
            {
                previousGold = totalGold;   //Set prev value for flipping uı animation
                totalGold -= turnCost;      // Decrease cost for the turn (fallback)
            }
        ...
        public void ClaimReward()
        {
            ...
            int rewardAmount = PiecesOfWheel[rewardIndex].rewardAmount * rewardMultiplier;
            ...
            switch (PiecesOfWheel[rewardIndex].rewardCategory)
            {
                case RewardEnum.Gold:
                    {
                        if (Wallet.Instance != null)
                            Wallet.Instance.AddCoins(rewardAmount);
                        else
                            totalGold += rewardAmount; // Fallback если Wallet не найден
                    }
                    break;
                case RewardEnum.Energy:
                    {
                        if (Wallet.Instance != null)
                            Wallet.Instance.AddEnergy(rewardAmount);
                        else
                            totalEnergy += rewardAmount;
                    }
                    break;
                case RewardEnum.Life:
                    {
                        if (Wallet.Instance != null)
                            Wallet.Instance.AddLives(rewardAmount);
                        else
                            totalLife += rewardAmount;
                    }
                    break;
                ...
                case RewardEnum.Gem4:
                    {
                        if (Wallet.Instance != null)
                            Wallet.Instance.AddGems(rewardAmount);
                        else
                        {
                            // Fallback для разных типов гемов
                            switch (PiecesOfWheel[rewardIndex].rewardCategory)
                            {
                                case RewardEnum.Gem1: totalGem1 += rewardAmount; break;
                                case RewardEnum.Gem2: totalGem2 += rewardAmount; break;
                                case RewardEnum.Gem3: totalGem3 += rewardAmount; break;
                                case RewardEnum.Gem4: totalGem4 += rewardAmount; break;
                            }
                        }
                    }
                    break;
                case RewardEnum.Money:
                    {
                        if (Wallet.Instance != null)
                            Wallet.Instance.AddCoins(rewardAmount); // Используем монеты как альтернативные золотые
                        else
